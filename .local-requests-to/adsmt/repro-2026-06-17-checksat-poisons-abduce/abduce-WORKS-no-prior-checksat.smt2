...
;; Function-Specs fail::p
(declare-fun req%fail!p. (Int Int) Bool)
...
(assert
 (forall ((x! Int) (y! Int)) (!
   ...
   :pattern ((req%fail!p. x! y!))
   :qid internal_req__fail!p._definition
)))
(declare-fun ens%fail!p. (Int Int) Bool)
(assert
 (forall ((x! Int) (y! Int)) (!
   (= (ens%fail!p. x! y!) (> (Add x! y!) 0))
   :pattern ((ens%fail!p. x! y!))
   :qid internal_ens__fail!p._definition
)))

;; Function-Def fail::p
;; /tmp/sound/fail.rs:2:15: 2:35 (#0)
(push)
 (declare-const x! Int)
 (declare-const y! Int)
 (assert
