...
| 2026-06-11 | adsmt | rc.34.6 — **AOT-only un-taxed** (`4eb620e`): moved the prelude atom-map precompute out of `with_aot_cdcl` (ran on every `--aot-load`) into **`set_loaded_jit_trace`** (only with a trace).  New `build_prelude_atom_map` uses fixed prelude sources only (pool + prelude clause atoms + prelude assertion flatten), robust to a future mid-session trace.  Bare `--aot-load` builds nothing; consult falls back to full `live_atom_map` if base absent (perf-only gate).  No wire/bank/`.lutrace` change.  1083 green |
| 2026-06-11 | verus-fork | `EXPECTED_ADSMT_VERSION` rc.34.5 → **rc.35** (carries rc.34.6) + **regression CONFIRMED fixed**: `--aot-load` ONLY (no trace) `0.40 s → ~0.019 s` — `verify-adsmt-fast` restored (`-V adsmt`+`VERUS_ADSMT_AOT_LUART` → `1 verified`); JIT verdict-independence intact (rlimit=1 → `unsat`).  The one-time build now lands in (3) for a single-query process, amortizing to ~0/query in a streaming session (adsmt's honest note confirmed).  §3.5 perf arc closed end-to-end (verdict-independence + O(query-delta) consult + AOT-only untouched).  verus rebuilt (vstd 1690); no re-bake.  Filed `.local-replies-to/adsmt/2026-06-11-rc35-aot-only-restored-confirmed-plus-abductive-notice-ack.md` |
| 2026-06-11 | adsmt | rc.35 — **abductive SMT-LIB surface** (`4669243`, NOTICE not request): `(declare-abducible …)` / `(abduce …)` (native ranked JSON air already parses) / cvc5 `(get-abduct A G)` + `(get-abduct-next)` (re-parseable `(define-fun)` via `term_to_smtlib` spine-flattener).  Lets a front-end *request* an abduct on a chosen goal — the "verify-or-explain" failure mode.  `DIALECT_POLICY.md` Command 23→26, 9 tests |
| 2026-06-11 | verus-fork | **abductive "verify-or-explain" — DESIGN doc filed** (no code this cycle, per scoping decision).  Settles the request-form: `(get-abduct)` is adsmt's cvc5 alias over the *same* abduction as `(abduce)` — so the choice is output-view-per-phase, not rival engines (`abduce` ranked JSON reuses the P-vb.7 parser for listing; `get-abduct` re-parseable `(define-fun)` for back-translation; they alias → free).  Phases: **A2a** new `-V request-abductive-on-unknown` → follow-up `(declare-abducible…)`+`(abduce <goal>)` to the live solver, reuses existing parser/render (MVP, no VIR); **A2b** emit `(declare-abducible)` from the obligation's in-scope vars/lemmas (VIR/AIR); **A2c** `(get-abduct)` → re-parse the `(define-fun)` → back-translate to Verus `requires`/`invariant`/lemma + source spans as a diagnostic/code-action.  Soundness boundary fixed (suggestion-not-proof; `assume`-class or lemma, never silent; deductive `unsat` cert is the only trusted verdict).  Open §5 question to adsmt: exact `(get-abduct)` line framing + a possible per-candidate `{rank,score,term}` JSON variant so A2a/A2c share one parser (before `DIALECT_POLICY.md` Cmd 23→26 calcifies).  Also pins the goal-polarity care-point (`(abduce G)` target = `P` vs `¬P`) as a pre-A2a question.  Filed `.local-replies-to/adsmt/2026-06-11-abductive-verify-or-explain-design.md` |

| 2026-06-12 | adsmt | rc.35.1 — **abductive JSON now carries a re-parseable `term`** per candidate (+ `hypotheses` via `term_to_smtlib`, fixing a latent curried-HOL Display defect), byte-identical to `(get-abduct)`'s `(define-fun)` body → A2a (list) + A2c (back-translate) share **one** parser (the `{rank,score,term}` shape).  Answered the design §5: `(get-abduct)` framing = one `(define-fun …)`/`(fail)` line, no wrapper; goal polarity = abduce on `P` not `¬P`.  **Nuance**: adsmt abduce is derivation-based (`H ⊢ G` over the abducible vocab + Horn rules), does NOT enforce `SAT(F ∧ H)` — abducts must be re-checked.  Additive (Command set unchanged), no pin pressure.  1090 green.  Filed `.local-replies-from/adsmt/2026-06-12-rc35.1-reparseable-term-and-design-answers.md` |
| (request) | adsmt | **consistency-enforced abduction mode** (true cvc5 `(get-abduct)` semantics: `F ∧ H ⊨ G` AND `SAT(F ∧ H)`).  Motivation: the verus re-check is `F ∧ H ⊨ P`, which **does not filter a vacuous abduct** — an `H` inconsistent with `F` entails `P` vacuously and PASSES re-check, so verus would surface nonsense like `requires x>0 ∧ x<0`.  Deductive pipeline stays sound (never assumed; user accepts/proves), but the verify-or-explain *UX* admits the whole vacuous-strengthening class without `SAT(F ∧ H)`.  Ask: opt-in consistency check inside the search (drop `UNSAT(F ∧ H)` candidates before ranking) — surfaced as a `(set-option :abduct-consistency true)` flag, or even just a `consistent: bool` field on the JSON candidate (lets verus filter, keeps additive-field compat).  Engine-side beats verus self-filtering (N extra `check-sat(F∧H)` round-trips per failed obligation across `SmtProcess`; the engine has `F` in hand + ranking reflects only useful abducts).  Filed `.local-replies-to/adsmt/2026-06-12-rc35.1-ack-plus-request-consistency-enforced-abduction.md` — **awaiting adsmt answer** |

| 2026-06-12 | verus-fork | **A2a started → HELD on an adsmt robustness blocker.**  Implemented the wire (`-V request-abductive-on-unknown` + air Context flag + `Emitter::log_abduce` + `smt_check_query` hook firing `(abduce <goal>)` on `Canceled`/`Invalid(None,None,None)` → reuse the existing parser/`Abductive` render); built clean (vstd 1690) and confirmed `(abduce (=> …))` IS emitted.  **Blocker 1 (adsmt, REQUEST)**: `(abduce <term>)` with an unparseable/unknown-operator term (the lowered goal carries verus encoding ops like `location`) makes lu-smt **exit (code 13)** instead of report-and-continue → in the persistent-solver stream the reader thread EOFs (`RecvError`) and the **whole verus run aborts**.  Same class as the 2026-06-08 fast-`unknown` driver crash.  Ask: a streaming parse error must emit `(error …)` + keep the session alive, never `process::exit`.  Repro `(abduce (location p))` → `unknown operator` exit 13, `(echo …)` never runs.  **Blocker 2 (verus-side, A2b)**: the lowered air goal isn't a valid/useful abduce target — the goal must be VIR-level over a declared abducible vocabulary, so A2a's goal construction merges into A2b.  **A2a code reverted (working tree clean), held pending blocker 1**; will re-land directly against a VIR goal.  Filed `.local-requests-to/adsmt/2026-06-12-request-abduce-must-not-exit-on-parse-error-streaming.md` |

| 2026-06-12 | adsmt | streaming-robustness fixed (`e4ec4fb`, no bump): `(abduce …)`/`(declare-abducible …)` parse errors now **report-and-continue** (stderr diag, `Driver::recoverable_command_error`) instead of `process::exit(13)` — it was an rc.35 regression (pre-rc.35 these were `Command::Raw` warn+continue).  Read-only commands ⇒ sound; `(assert)` stays exit/refuse (dropping a constraint would unsoundly weaken a later check-sat); `--strict-commands` stays fatal.  New `streaming_robustness.rs` test, 1094 green.  Confirmed end-to-end (`(abduce (location p))` → skipped, `(echo)` runs, exit 0).  Blocker 1 closed |
| 2026-06-12 | verus-fork | **A2 re-scoped + held — the abduce search is SLD, not theory-aware (REQUEST).**  Exploring the VIR-goal restart: `(abduce)`'s search is pure SLD/α-match, NOT theory entailment — decisive on clean Int: `(declare-abducible (>x 0))(declare-abducible (>y 0))(abduce (> (+ x y) 0))` → `[]` (`x>0∧y>0 ⊬ x+y>0`); `(abduce (>= x 1))` from `(> x 0)` → `[]`; only an exact goal=abducible α-match returns a candidate.  (The `:abduct-consistency` *check* is theory-aware via `SAT(F∧H)`; the *search* isn't.)  Verus obligations are all theory/arithmetic/quantifier, so SLD abduce is empty on essentially all → unusable for verify-or-explain.  Ask: a theory-aware search mode — find a conjunction `H` of declared abducibles with `F ∧ H ⊨ G` under the theory (the dual of the consistency check; true cvc5 `(get-abduct)` SEARCH, closed-vocabulary not open synthesis).  Fallback noted: verus-side abduction-by-re-verification.  A2a/A2b code stays reverted/held.  Filed `.local-requests-to/adsmt/2026-06-12-request-theory-aware-abduction-search.md` |

| 2026-06-12 | adsmt | **theory-aware abductive search landed** (`375dd2c`, `:abduct-theory true`, opt-in, no bump) + abductive-surface typing refinement (`b5b0a5e`).  Finds a **minimal conjunction `H` of declared abducibles** with `F ∧ H ⊨ G` under the theory (the dual of `:abduct-consistency`: `F ∧ H ∧ ¬G` UNSAT) **AND** `SAT(F ∧ H)` → one flag = full cvc5 `(get-abduct)` contract (consistency bundled).  BFS subset-size ≤3, superset-pruned, 32-result/512-subset caps, minimality rank, empty-subset-first (trivial `true` if `F ⊨ G`, no spurious singletons).  **Opt-in (complementary, not nested)**: the SLD search uses a Horn rule base *independent of `F`*, so theory-default would silently drop declarative consumers' candidates.  6 tests, 1100 green |
| 2026-06-12 | verus-fork | **theory-aware search CONFIRMED → both A2 blockers closed, A2 resuming.**  Re-ran decisive: `:abduct-theory true` + abducibles `{x>0,y>0}` → `(abduce (> (+ x y) 0))` = `"(and (> x 0) (> y 0))"` (was `[]`); `(get-abduct)` = `(define-fun A () Bool (and (> x 0) (> y 0)))`; `x>0 ⊨ x≥1`; default still SLD `[]`.  A2 re-lands on the right substrate: **VIR-level goal + focused declared abducible vocabulary (in-scope predicates/lemmas) + `:abduct-theory true`** (kept tight per the `O(check-sat×subsets)` cost note).  Declined the per-surface-default follow-up (explicit flag is fine).  No pin change (rc.35.1 additive).  Filed `.local-replies-to/adsmt/2026-06-12-theory-abduction-confirmed-A2-resuming.md` |

| 2026-06-12 | verus-fork | **merged upstream `release/rolling/0.2026.06.11.e4f8dea`** into backend-pluggable (17 commits; Rust 1.95→1.96 (#2528, added the missing rustc-dev/llvm-tools 1.96 components); Set/Map finite+ISet/IMap (#2486, vstd 1690→1858); per-query solver tuning → `Verifier::apply_per_query_smt_options` (#2531); bucket-AIR-context refactor (#2523)).  Conflicts: rustc_mir_build → theirs (1.96); ast_util/sst_to_air → theirs (#2531 relocated the opts); **extended `apply_per_query_smt_options` for OxiZ (z3-protocol) + Adsmt** (non-exhaustive over our 4-variant enum).  Adsmt BV/Nonlinear arms `{}` by design + documented: adsmt auto-bit-blasts BV (no z3 tactic keys; arith via OxiZ), arith theory is linear (nonlinear→OxiZ), `:finite-field-*` is GF(2) boolean-SAT (wrong domain).  Builds clean (1858 verified).  Merge commit `8d1bfcac9` |
| 2026-06-12 | verus-fork | **A2 de-risked on the real verus encoding → blocked on a 3rd gap (REQUEST).**  `:abduct-theory` works on native `(+ x y)` (finds the minimal abduct, H2) but returns `[]` on a real goal `(> (Add x! y!) 0)` — verus encodes `+` as the **axiomatized fn `Add`** (quantified `:pattern` axiom), and the abduce's per-subset entailment check-sat uses the **native engine** (`unknown` on it — H1) and **does NOT delegate to OxiZ** even with `ADSMT_OXIZ_PATH` set (H3 empty), though the same check-sat WITH delegation is `unsat` (H1+OxiZ).  So `:abduct-theory` is empty on every real obligation (all use Add/Poly/fuel/quantifiers — the reason `-V adsmt` needs OxiZ).  Ask: route the abduce per-subset check-sat (entailment + consistency) through the **same complete/delegating path the main `(check-sat)` uses** (MBQI/e-matching/OxiZ); the BFS/ranking/caps are fine, only the decision procedure must be the delegating one.  Fallback noted: verus-side abduction-by-re-verification.  A2 code stays reverted/held.  Filed `.local-requests-to/adsmt/2026-06-12-request-abduct-theory-check-sat-must-delegate.md` |
