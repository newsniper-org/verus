...
| (future) | verus-fork | **abductive "verify-or-explain" wire** (rc.35 consume) — Verus-side, scoped as its own P2-scale cycle: (1) trigger `(get-abduct G)` on a failed/`unknown` obligation; (2) emit `(declare-abducible …)` for in-scope vars/lemmas; (3) back-translate the abduct to Verus surface (`requires`/`invariant`/lemma) with source spans, surfaced as a diagnostic/code-action.  **Soundness boundary**: an abduct is a suggestion, never a proof — a new obligation the user accepts (`assume`-class) or proves, never silently assumed; the deductive `unsat` cert stays the only trusted verdict.  Acked at the reply above |
