...
;; Function-Specs neqzero::p
(declare-fun ens%neqzero!p. (Int) Bool)
(assert
 (forall ((x! Int)) (!
   (= (ens%neqzero!p. x!) (not (= x! 0)))
   :pattern ((ens%neqzero!p. x!))
   :qid internal_ens__neqzero!p._definition
)))

;; Function-Def neqzero::p
;; /tmp/voc2/neqzero.rs:2:15: 2:27 (#0)
(declare-const x! Int)
(push)
